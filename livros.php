...
?>
    ...
    <?php if (count($livros) > 0): ?>
        ...
        <!-- View em Cards (com capa do livro) -->
<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 25px; margin-bottom: 30px;">
    <?php foreach ($livros as $livro): 
        ...
        $capa = !empty($livro['capa']) && file_exists($livro['capa']) 
            ? $livro['capa'] 
            : 'imagens/sem_capa.png';
    ?>
        <div class="card" style="display: flex; flex-direction: column; overflow: hidden; border-radius: 10px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); <?= !$disponivel ? 'opacity: 0.7;' : '' ?>">
            ...
            <div style="padding: 15px; flex: 1; position: relative;">
                ...
                <p style="color: #666; margin: 10px 0 5px 0;">
                    <strong>Disponíveis:</strong> <?= $livro['quantidade_disponivel'] ?> / <?= $livro['quantidade_total'] ?>
                </p>
            ...
            <!-- Ações -->
            <div style="display: flex; justify-content: space-around; padding: 10px; border-top: 1px solid #eee; background: #fafafa;">
                <a href="livro_editar.php?id=<?= $livro['id'] ?>" class="btn btn-warning btn-small">✏️ Editar</a>
                <?php if ($disponivel): ?>
                    <a href="emprestimo_novo.php?livro_id=<?= $livro['id'] ?>" class="btn btn-success btn-small">📋 Emprestar</a>
                <?php endif; ?>
                <a href="livro_excluir.php?id=<?= $livro['id'] ?>" class="btn btn-danger btn-small confirm-delete">🗑️ Excluir</a>
            </div>
        </div>
    <?php endforeach; ?>
</div>
